...
class Customer {
    ...
    mobileNumber;
    balance;
    constructor(firstName, lastName, gender, age, mobileNumber, initialBalance) {
...
async function main() {
    const accountInfo = await inquirer.prompt([
        ...
        },
        {
            type: 'number',
            name: 'initialBalance',
            message: "Please enter your initial account balance:"
        }
    ]);
    const customer = new Customer(accountInfo.firstName, accountInfo.lastName, accountInfo.gender, accountInfo.age, accountInfo.mobileNumber, accountInfo.initialBalance);
    ...
    if (transactionChoice.transaction === 'Check Balance') {
        console.log(`Your current balance is: $${customer.checkBalance()}`);
    }
        ...
        const debitAmount = await inquirer.prompt([
            {
                type: 'number',
                name: 'amount',
                message: 'Please enter the debit amount:'
            }
        ]);
